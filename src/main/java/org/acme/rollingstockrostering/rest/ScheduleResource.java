...
public class ScheduleResource {
    ...
    @Path("/solve")
    public void solve() {
        // If no current data, generate default
        if (currentJobId == null || !scheduleMap.containsKey(currentJobId)) {
        ...
        // Start solving with current data
    ...
    public RollingStockSchedule generateData(
            @QueryParam("trainCount") Integer trainCount,
            @QueryParam("startHour") Integer startHour,
            @QueryParam("endHour") Integer endHour,
            @QueryParam("intervalHours") Integer interval) {
        ...
        // Stop current solving if exists
        if (currentJobId != null) {
            solverManager.terminateEarly(currentJobId);
        }
        
        // Create new job ID and save schedule
        currentJobId = UUID.randomUUID().toString();
        scheduleMap.put(currentJobId, schedule);
