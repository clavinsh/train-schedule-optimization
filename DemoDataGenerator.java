 ...
 * 
 * Creates:
 * - 148 stations with real Latvian coordinates
 * - 10 routes connecting stations
 * - 30-40 trains with different capacities
 * - 5 depots
 * - Hourly passenger demand
 * - Unassigned departure times (planning entities)
 * 
 * Travel time between stations is calculated using Haversine formula
...
public class DemoDataGenerator {
    ...
    // Average train speeds in km/h
    private static final int ELEKTRO_SPEED_KMH = 80;  // Electric train average speed
    private static final int DIZEL_SPEED_KMH = 60;    // Diesel train average speed
    private static final int DEFAULT_SPEED_KMH = 70;  // Default speed for calculations
    ...
    private static RollingStockSchedule generateDataset(int startHour, int endHour, int intervalHours) {
        ...
        // Create station lookup map for coordinate-based calculations
        ...
        // Now using coordinate-based travel time calculation
        List<AtiesanasLaiks> atiesanasLaiki = generateAtiesanasLaiki(
                stacijas, marsruti, cilvekuPieprasijumi, stacijuMapa,
    ...
    /**
     * Calculate travel time between two stations based on coordinates
     * @param stacija1 departure station
     * @param stacija2 arrival station
     * @param speedKmH average speed in km/h
     * @return travel time in minutes
     */
    public static long calculateTravelTimeMinutes(Stacija stacija1, Stacija stacija2, int speedKmH) {
        GeoCoordinates coord1 = stacija1.getKoordinatas();
        GeoCoordinates coord2 = stacija2.getKoordinatas();
        ...
        // Add 20% to account for non-straight tracks (curves, detours)
        double adjustedDistance = distanceKm * 1.2;
        
        // Calculate time in hours, then convert to minutes
        double timeHours = adjustedDistance / speedKmH;
        long timeMinutes = Math.round(timeHours * 60);
        
        // Minimum 2 minutes between adjacent stations
        return Math.max(timeMinutes, 2);
    }
    
    /**
     * Calculate cumulative travel time from first station to given station index
     * @param marsruts the route
     * @param stacijuMapa station lookup map
     * @param targetStationIndex index of target station in route
     * @param speedKmH average speed
     * @return cumulative travel time in minutes from route start
     */
    public static long calculateCumulativeTravelTime(
            Marsruts marsruts, 
        ...
        for (int i = 0; i < targetStationIndex && i < stacijuIds.size() - 1; i++) {
            Stacija current = stacijuMapa.get(stacijuIds.get(i));
            Stacija next = stacijuMapa.get(stacijuIds.get(i + 1));
            
            if (current != null && next != null) {
                totalMinutes += calculateTravelTimeMinutes(current, next, speedKmH);
                totalMinutes += 2; // Add 2 minutes stop time at each station
            }
        }
    ...
    /**
     * Generate 148 stations in Latvia with real coordinates
     */
    private static List<Stacija> generateStacijas() {
        ...
        stacijas.add(createStacija(29L, "Krustpils", 56.5095, 26.1840, Arrays.asList(28L, 30L, 39L, 54L)));
     ...
     * 
     * Uses coordinate-based travel time calculation between stations
     */
    private static List<AtiesanasLaiks> generateAtiesanasLaiki(
        ...
        for (Marsruts marsruts : marsruti) {
            ...
            for (int hour = startHour; hour <= endHour; hour += intervalHours) {
                ...
                // Create departure for each station on the route with calculated travel times
                List<Long> stacijuIds = marsruts.getStacijas();
                LocalTime currentTime = startTime;
                
                for (int stationIndex = 0; stationIndex < stacijuIds.size(); stationIndex++) {
                    Long stacijaId = stacijuIds.get(stationIndex);
                    
                    // Calculate travel time from previous station (if not first station)
                    if (stationIndex > 0) {
                        Stacija prevStacija = stacijuMapa.get(stacijuIds.get(stationIndex - 1));
                        Stacija currStacija = stacijuMapa.get(stacijaId);
                        
                        if (prevStacija != null && currStacija != null) {
                            long travelMinutes = calculateTravelTimeMinutes(
                                    prevStacija, currStacija, DEFAULT_SPEED_KMH);
                            long stopMinutes = 2; // 2 minutes stop at previous station
                            currentTime = currentTime.plusMinutes(travelMinutes + stopMinutes);
                        }
                    }
                    
                    int cilvekuDelta = getExpectedPassengers(
                            stacijaId, marsruts.getId(), currentTime, cilvekuPieprasijumi
                    );
    ...
    private static int getRouteOffset(Long marsrutaId) {
        switch (marsrutaId.intValue()) {
            ...
            case 2: return 15;  // Rīga-Tukums II: 06:15, 08:15, 10:15...
            case 3: return 30;  // Rīga-Valga: 06:30, 08:30, 10:30...
            case 4: return 45;  // Rīga-Skulte: 06:45, 08:45, 10:45...
            case 5: return 5;   // Rīga-Ogre: 06:05, 08:05, 10:05...
            case 6: return 10;  // Rīga-Indra: 06:10, 08:10, 10:10...
            case 7: return 20;  // Rīga-Zilupe: 06:20, 08:20, 10:20...
            case 8: return 25;  // Rīga-Gulbene: 06:25, 08:25, 10:25...
            case 9: return 35;  // Rīga-Jelgava: 06:35, 08:35, 10:35...
            case 10: return 40; // Rīga-Jūrmala: 06:40, 08:40, 10:40...
            default: return 0;
        }
    }
     ...
     */
    public static double calculateRouteDistanceKm(Marsruts marsruts, Map<Long, Stacija> stacijuMapa) {
        List<Long> stacijuIds = marsruts.getStacijas();
        ...
        for (int i = 0; i < stacijuIds.size() - 1; i++) {
            ...
            if (current != null && next != null) {
                GeoCoordinates c1 = current.getKoordinatas();
                GeoCoordinates c2 = next.getKoordinatas();
    ...
    /**
     * Calculate total route travel time in minutes
     * Useful for debugging and statistics
     */
    public static long calculateRouteTravelTimeMinutes(Marsruts marsruts, Map<Long, Stacija> stacijuMapa, int speedKmH) {
        List<Long> stacijuIds = marsruts.getStacijas();
        long totalMinutes = 0;
